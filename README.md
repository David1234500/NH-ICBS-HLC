...
```
                 ┌─────────────────────────────────────────┐
                 │  CBS (high level)                        │
                 │  - finds conflicts between vehicle paths │
                 │  - adds spatio-temporal constraints      │
                 │  - replans the affected vehicle          │
                 └───────────────┬─────────────────────────┘
                                 │ per-vehicle (re)planning jobs
                 ┌───────────────▼─────────────────────────┐
                 │  A* (low level)                          │
                 │  - searches a discretized state lattice  │
                 │  - expands precomputed motion primitives │
                 │  - respects CBS constraints as obstacles │
                 └───────────────┬─────────────────────────┘
                                 │ uses
                 ┌───────────────▼─────────────────────────┐
                 │  Motion primitive graph (offline)        │
                 │  - kinematically feasible short maneuvers │
                 │  - indexed by (heading, speed)           │
                 │  - generated by brute-force or NLopt      │
                 └─────────────────────────────────────────┘
```
