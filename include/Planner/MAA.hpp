...
struct CompareMAANode {
    bool operator()(const MAANode& lhs, const MAANode& rhs) const {
        return lhs.g + lhs.h > rhs.g + rhs.h;
    }
};

std::vector<Path> reconstructMAAPaths(const std::unordered_map<std::vector<Position>, std::vector<Position>>& cameFrom,
                                      const std::vector<Position>& start,
                                      const std::vector<Position>& goal) {
    std::vector<Path> paths(start.size());
    std::vector<Position> current = goal;
...
bool isValidPosition(const Grid& grid, const Position& pos) {
    return pos.x >= 0 && pos.x < grid.size() && pos.y >= 0 && pos.y < grid[0].size() && !grid[pos.x][pos.y];
}

bool isConflicting(const std::vector<Position>& positions) {
...
std::vector<Path> findMultiAgentPathsMAA(const Grid& grid, const std::vector<Agent>& agents) {
    ...
    std::priority_queue<MAANode, std::vector<MAANode>, CompareMAANode> openList;
    std::unordered_map<std::vector<Position>, MAANode> allNodes;
    std::unordered_map<std::vector<Position>, std::vector<Position>> cameFrom;
    MAANode startNode{start, 0, manhattanDistanceSum(start, goal), {}};

    allNodes[start] = startNode;
    openList.push(startNode);

    while (!openList.empty()) {
        MAANode current = openList.top();
        openList.pop();

        if (current.pos == goal) {
            return reconstructMAAPaths(cameFrom, start, goal);
        }

        for (size_t agentIdx = 0; agentIdx < agents.size(); ++agentIdx) {
            std::vector<Position> neighbors = getNeighbors(grid, current.pos[agentIdx]);
            neighbors.push_back(current.pos[agentIdx]); // Stay at the current position

            for (const Position& neighbor : neighbors) {
                std::vector<Position> nextPositions = current.pos;
                nextPositions[agentIdx] = neighbor;

                if (!isValidPosition(grid, neighbor) || isConflicting(nextPositions)) {
                    continue;
                }

                int tentativeG = current.g + 1;

                if (allNodes.find(nextPositions) == allNodes.end() || tentativeG < allNodes[nextPositions].g) {
                    cameFrom[nextPositions] = current.pos;
                    MAANode nextNode{nextPositions, tentativeG, manhattanDistanceSum(nextPositions, goal), current.pos};

                    allNodes[nextPositions] = nextNode;
...
int main() {
    ...
    std::vector<Path> paths = findMultiAgentPathsMAA(grid, agents);
